...
export class ChatService{
    ...
    async create(user: User, request: CreateChatRequest): Promise<WebResponse<ChatResponse[]>> {
        const createChatRequest: CreateChatRequest = await this.validationService.validate(chatValidation.CREATE, request);
        await this.chatSessionService.checkChatSessionMustExists(createChatRequest.session_id, user.id);
        ...
        const aiResponse = await this.groqService.getChatCompletion(createChatRequest.message);
